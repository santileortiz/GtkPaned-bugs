//gcc child_does_not_show.c -o child_does_not_show `pkg-config --cflags --libs gtk+-3.0`
/*
...
int main (int argc, char *argv[])
{
    ...
    // These lines seem to cause the problem but I have no idea why.
#if 1
    // NOTE: Not both are required to cause the issue, having any of them causes
    // top_pane to not draw until some separator moves.
    // NOTE: Not using fix_gtk_paned_new() and letting size warning appear still
    // causes the issue. But setting the sidebar to something simple that makes
    // the fix unnecessary, does make the children appear. So these things may
    // be somehow related.
    gtk_widget_set_valign (top_pane, GTK_ALIGN_CENTER);
    gtk_widget_set_halign (top_pane, GTK_ALIGN_CENTER);
#endif
